...
export const authenticate = async (req, res, next) => {
  try {
    ...
    const tokenVersion = await cache.get(`token:ver:${user.id}`);
    if (tokenVersion && decoded.version !== undefined && decoded.version < tokenVersion) {
...
export const generateTokens = (userId, version = 0) => {
  const accessToken = jwt.sign({ userId, version }, process.env.JWT_SECRET, { expiresIn: process.env.JWT_EXPIRES_IN || '15m' });
  const refreshToken = jwt.sign({ userId, version }, process.env.JWT_REFRESH_SECRET, { expiresIn: process.env.JWT_REFRESH_EXPIRES_IN || '7d' });
